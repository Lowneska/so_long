...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/09/07 22:31:54 by skhali            #+#    #+#             */
/*   Updated: 2022/09/09 20:47:40 by skhali           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	display_moves(t_map *map)
{
	...
	mlx_string_put(map->image->mlx, map->image->mlx_win, map->h_len * 64 - 10,
		map->h_len * 64 - 20, 0xFFFFFF, "moves :");
	mlx_string_put(map->image->mlx, map->image->mlx_win, map->h_len * 64,
		map->h_len * 64, 0xFFFFFF, moves);
	free(moves);
}
...
void	put_vue(int vue, t_map *map, int a, int b)
{
	...
	if (vue == 3)
		mlx_put_image_to_window(map->image->mlx, map->image->mlx_win,
			map->image->profil_b, b, a);
	if (vue == 4)
		mlx_put_image_to_window(map->image->mlx, map->image->mlx_win,
			map->image->profil_a, b, a);
}
...
int	player_moves(int keycode, t_map *map)
{
	...
	mlx_put_image_to_window(map->image->mlx, map->image->mlx_win,
		map->image->wall_down, map->h_len * 64, (map->h_len - 1) * 64);
	display_moves(map);
	return (0);
}
